# **ProgrammingForPsychologists_Assignment**
# Python program indicating the brain regions associated with a psychological phenomenon
- Korosh Rajaei  
- Date of completion: November 25, 2025  
- [Link to the Neurosynth reward anticipation page](https://neurosynth.org/analyses/terms/reward%20anticipation/)  
## **Project Description**
In this project, I have written a Python program that shows the brain areas activated in reward anticipation (data from Neurosynth, meta-analysis of 92 studies). However, I wrote the program (functional programming paradigm) in a way that it is flexible to be used with different psychological data from Neurosynth. Here, I show that reward anticipation is mostly associated with the ventral striatum, nucleus accumbens, and ventral tegmental area.

---

## **Table of Contents**
- [Project Description](#project-description)
- [Data Used](#data-used)
- [Notebook File](#notebook-file)
- [Python Packages Used](#python-packages-used)

---

## **Data Used**  
The data and files were downloaded from Neurosynth.  
| File Name       | Description |
|-----------------|-------------|
| `anatomical.nii.gz`      | Contains the anatomical scan of the brain from Neurosynth. |
| `reward anticipation_uniformity-test_z_FDR_0.01.nii.gz`      | Contains the brain functional data of reward anticipation from Neurosynth. |

---

## **Notebook File**

| Notebook | Purpose |
|----------|---------|
| `Prog4Psy_HomeAssignment_Notebook.ipynb` | Main notebook containing the codes, visualizations, and analysis. |

---

## **Python Packages Used**
- nibabel
- nilearn
- matplotlib
- numpy
- pathlib

---

## **Sources**
- https://nipy.org/nibabel/nibabel_images.html
- https://docs.python.org/3/library/pathlib.html
- https://www.geeksforgeeks.org/python/python-next-method/
- https://nilearn.github.io/dev/modules/generated/nilearn.plotting.plot_stat_map.html

---


